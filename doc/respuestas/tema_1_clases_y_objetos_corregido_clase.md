...
Ventajas heap: La memoria es dinámica, se decide lo que ocupa en tiempo de ejecución
La vida de los objetos del heap no  dependen de la vida de la función que los usa
Problemas del heap y como se solucionan:
 Alguien tiene que encargarse de eliminar la memoria no usada del heap{
    Manualmente--> Díficil y propenso a bugs( "Memory leak")
    Con recolector de basura--> Elimina objetos no accesibles direcra o indirectamente desde el stack
     "Desventaja recolector":Rendimiento
}
...
Static{
    -Permite usar un método o atributo sin necesidad de tener una instancia de la clase se antepone el nombre de la CLASE no de una instancia
        ej: {
            - main
            -Integer.parseInt(...)
            -Math.sqrt(...)
    ...
    -En atributos, solo se guardan en un único lugar de la memoria
}
Final{
    -Constante
    -No se le puede asignar otro valor en el futuro
}
